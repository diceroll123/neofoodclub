    ...
    for (let arenaIndex = 0; arenaIndex < 5; arenaIndex++) {
      ...
      for (let pirateIndex = 0; pirateIndex < 4; pirateIndex++) {
        ...
        for (let foodIndex = 0; foodIndex < 10; foodIndex++) {
          ...
          favorites[arenaIndex][pirateIndex] += POSITIVE_FAS[pirateId][foodId];
          allergies[arenaIndex][pirateIndex] -= NEGATIVE_FAS[pirateId][foodId];
        }
      }
    }
  }
  ...
  for (let arenaIndex = 0; arenaIndex < 5; arenaIndex++) {
    ...
    for (let pirateIndex = 0; pirateIndex < 4; pirateIndex++) {
      fas[arenaIndex][pirateIndex] =
        favorites[arenaIndex][pirateIndex] + allergies[arenaIndex][pirateIndex];
    }
  }
  return fas;
}
...
export function computeLogitProbabilities(roundData) {
  ...
  for (let arenaIndex = 0; arenaIndex < 5; arenaIndex++) {
    ...
    for (let pirateIndex = 1; pirateIndex <= 4; pirateIndex++) {
      returnValue.prob[arenaIndex][pirateIndex] =
        capabilities[pirateIndex] / capabilities[0];
      returnValue.used[arenaIndex][pirateIndex] =
        returnValue.prob[arenaIndex][pirateIndex];
    }
  }
...
export function calculateArenaRatios(customOdds) {
  let arenas = [];
  for (let arenaIndex = 0; arenaIndex < 5; arenaIndex++) {
    let ratio = 0;
    for (let pirateIndex = 1; pirateIndex <= 4; pirateIndex++) {
